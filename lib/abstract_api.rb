...
class AbstractApi
  ...
  def post_with_body(url, payload, content_type='application/json', suppress_debug_log=false, with_retry=true)
    request = Net::HTTP::Post.new("#{@protocol}://#{@auth_prefix}#{@host}:#{@port}/#{url}")
  ...
  def put(url, payload, suppress_debug_log=false, with_retry=true)
    request = Net::HTTP::Put.new("#{@protocol}://#{@auth_prefix}#{@host}:#{@port}/#{url}")
    request.body = payload
    if with_retry
      response = do_request_with_retry request, suppress_debug_log
    else
  ...
  def delete(url, suppress_debug_log=false, with_retry=true)
    request = Net::HTTP::Delete.new("#{@protocol}://#{@auth_prefix}#{@host}:#{@port}/#{url}")
    if with_retry
      response = do_request_with_retry request, suppress_debug_log
    else
      response = do_request request, suppress_debug_log
  ...
  def get(url, params=Hash.new, suppress_debug_log=true, with_retry=true)
    inject_default_params(params)
    real_url = "#{@protocol}://#{@auth_prefix}#{@host}/#{url}?".concat(params.collect{|k,v| "#{k}=#{CGI::escape(v.to_s)}"}.join("&"))
  ...
  def do_request_with_retry(request, suppress_debug_log=true, retry_count=1)
 ...
 def do_request(request, suppress_debug_log=true)
    net = Net::HTTP.new(@host, @port)
    net.use_ssl = @use_ssl
    if @use_ssl
      net.verify_mode = OpenSSL::SSL::VERIFY_PEER
      net.ca_file = 'config/curl_cacert.pem'
      if net.respond_to? :enable_post_connection_check
