/*!
 * Retention Factory analytics, based on Piwik - Web Analytics basically unmodified except
 *   for changing all references to "piwik" to "ret_factory" to avoid collisions with clients who
 *   are also using piwik.
 *
...
/*global _retf:true */
/*members push */
/*global RetFactory:true */
/*members addPlugin, getTracker, getAsyncTracker */
/*global RetFactory_Overlay_Client */
/*members initialize */
/*global define */
/*members amd */

// asynchronous tracker (or proxy)
if (typeof _retf !== 'object') {
    _retf = [];
}

// RetFactory singleton and namespace
if (typeof RetFactory !== 'object') {
    RetFactory = (function () {
        'use strict';
            ...
            for (i = 0; i < arguments.length; i += 1) {
                ...
                if (isString(f)) {
                	if (asyncTracker[f]){
	                	...
	                	throw "Sorry, the method " + f + " doesn't exist.  Please check for typos.";
	                }
                } else {
        ...
        function getRetFactoryUrlForOverlay(trackerUrl, apiUrl) {
            if (apiUrl) {
                return apiUrl;
            }

            if (trackerUrl.slice(-9) === 'ret_factory.php') {
        ...
        function isOverlaySession(configTrackerSiteId) {
            var windowName = 'RetFactory_Overlay';

            // check whether we were redirected from the ret_factory overlay plugin
        ...
        function injectOverlayScripts(configTrackerUrl, configApiUrl, configTrackerSiteId) {
            var windowNameParts = windowAlias.name.split('###'),
                ...
                ret_factoryUrl = getRetFactoryUrlForOverlay(configTrackerUrl, configApiUrl);

            loadScript(
                ret_factoryUrl + 'plugins/Overlay/client/client.js?v=1',
                function () {
                    RetFactory_Overlay_Client.initialize(ret_factoryUrl, configTrackerSiteId, period, date);
                }
            );
        }
...
/*</DEBUG>*/
                ...
                // Campaign names
                configCampaignNameParameters = [ 'pk_campaign', 'ret_factory_campaign', 'utm_campaign', 'utm_source', 'utm_medium' ],

                // Campaign keywords
                configCampaignKeywordParameters = [ 'pk_kwd', 'ret_factory_kwd', 'utm_term' ],
            ...
            /*
             * Send image request to RetFactory server using GET.
            ...
            /*
             * POST request to RetFactory server using XMLHttpRequest.
             */
            ...
            function getCookieName(baseName) {
                // NOTE: If the cookie name is changed, we must also update the RetFactoryTracker.php which
            ...
            /**
             * Returns the URL to call ret_factory.php,
            ...
            function getRequest(request, customData, pluginMethod, currentEcommerceOrderTs) {
                ...
                // do not send charset if it is utf8 since it's assumed by default in RetFactory
                ...
                if (ecommerceItems) {
                    ...
                    for (sku in ecommerceItems) {
                        if (Object.prototype.hasOwnProperty.call(ecommerceItems, sku)) {
							...
							if (!isDefined(ecommerceItems[sku][7])
									|| String(ecommerceItems[sku][7]).length === 0) {
								ecommerceItems[sku][7] = sku;			
							}	
            ...
            function getClassesRegExp(configClasses, defaultClass) {
                var i,
                    classesRegExp = '(^| )(ret_factory[_-]' + defaultClass;
...
/*
                ...
                if (linkPattern.test(className)) {
                    // class attribute contains 'ret_factory_link' (or user's override)
                    linkType = 'link';
                } else if (downloadPattern.test(className)) {
                    // class attribute contains 'ret_factory_download' (or user's override)
                    linkType = 'download';
...
/*</DEBUG>*/
                 ...
                 *   1) Call JSON2.stringify(ret_factoryTracker.getAttributionInfo())
                ...
                /**
                 * Specify the RetFactory server URL
                 *
                ...
                /**
                 * Appends the specified query string to the ret_factory.php?... Tracking API URL
                 *
                ...
                /**
                 * Set the URL of the RetFactory API. It is used for Page Overlay.
                 ...
                 * When clicked, RetFactory will log the click automatically.
                 *
...
/*                        trackCallback(function () {
                ...
                /**
                 * Ties the current visitor id to an internal user so we can better track over time and across browsers
                 *
                 * @param string userId any unique internal identifier for this user
                 * @param string email (optional) the email address of this user
                 * @param string createdAt (optional) the timestamp when this user was originally created, in seconds since the Unix epoch
                 * @param string otherProperties (optional) any other info about the user to keep track of
                 */
                identifyVisitor: function (userId, email, createdAt, otherProperties) {
                    trackCallback(function () {
                    	if (!isDefined(otherProperties)){
                    		otherProperties = {}
                    	}
                    	otherProperties.email = email;
                    	otherProperties.createdAt = createdAt;
                        logEvent('ret_factory_internal', 'identify', userId, null,otherProperties);
                    });
                },
                 ...
                 *
                 * Tracking Product/Category page views will allow RetFactory to report on Product & Categories
                 * conversion rates (Conversion rate = Ecommerce orders containing this product or category / Visits to the product or category)
                 *
                 * @param string sku Item's SKU code being viewed
                 * @param string name Item's Name being viewed
                 * @param string category Category page being viewed. On an Item's page, this is the item's category
                 * @param float price Item's display price, not use in standard RetFactory reports, but output in API product reports.
                 */
                 ...
                 * @param string product_image_url (optional) The thumbnail image for the product.  If left off, some functionality of RetentionEngine won't work
                 * @param string product_purchase_url (optional) The landing page for the product purchased
                 * @param string product_id (optional)  The parent product id.  For example, if you have one shirt with three colors, each color would have a different sku but all of them would have the same product id.  if left off, defaults to the sku
                 */
                addEcommerceItem: function (sku, name, category, price, quantity, product_image_url, product_purchase_url, product_id) {
                    if (sku.length) {
                        ecommerceItems[sku] = [ sku, name, category, price, quantity, product_image_url, product_purchase_url, product_id ];
                    }
                },
                 ...
                 * All revenues (grandTotal, subTotal, tax, shipping, discount) will be individually summed and reported in RetFactory reports.
                 * Parameters orderId and grandTotal are required. For others, you can set to false if you don't need to specify them.
                 *
                 * @param string|int orderId (required) Unique Order ID.
                 *                   This will be used to count this order only once in the event the order page is reloaded several times.
                 *                   orderId must be unique for each transaction, even on different days, or the transaction will not be recorded by RetFactory.
        ...
        /************************************************************
         * Proxy object
         * - this allows the caller to continue push()'ing to _retf
        ...
        // initialize the RetFactory singleton
        ...
        for (iterator = 0; iterator < _retf.length; iterator++) {
            if (_retf[iterator][0] === 'setTrackerUrl'
                    || _retf[iterator][0] === 'setSiteId') {
                apply(_retf[iterator]);
                delete _retf[iterator];
            }
        }
        ...
        RetFactory = {
             ...
             *
             * @param string ret_factoryUrl
             * @param int|string siteId
             * @return Tracker
             */
            getTracker: function (ret_factoryUrl, siteId) {
                return new Tracker(ret_factoryUrl, siteId);
            },
        ...
        // Expose RetFactory as an AMD module
        if (typeof define === 'function' && define.amd) {
            define('ret_factory', [], function () { return RetFactory; });
        }
